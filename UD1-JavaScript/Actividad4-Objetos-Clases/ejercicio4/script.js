...
class Clase {
    ...
    getAlumnoPorNombre(nombre){

        let texto = "";
        for(let alumno of this.alumnos){

            // Si el alumno tiene el nombre qaue le paso por parámetros, que es el nombre buscado, entonces devolvemos su info
            if(alumno.getNombre() === nombre){
                texto += "<h3>Alumno econtrado</h3>"
        ...
        // Si acaba de recorrer los alumnos y no lo encuentra entonces devuelve un mensaje de alumno no encontrado
        return `Alumno con el nombre ${nombre} no encontrado`
...
function buscarAlumnoPorNombre(){
    const pAlumnoBuscado = document.getElementById("palumnoBuscado");
    const valorInputNombre = document.getElementById("inputNombre").value;

    pAlumnoBuscado.innerHTML = clase.getAlumnoPorNombre(valorInputNombre);
}